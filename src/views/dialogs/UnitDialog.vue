...
const validateField = (field) => {
  errors.value[field] = "";
  if (field === "unitName" && !formData.value.unitName) {
    errors.value.unitName = "Đơn vị tính không được để trống.";
  }
};

const validateAll = () => {
  validateField("unitName");
  return !errors.value.unitName;
};
...
const save = async () => {
  ...
  try {
    ...
    await unitStore.addUnit(formData.value.unitName);
    toast.success("Thêm đơn vị tính thành công!");
    emit("saved", formData.value.unitName);
    close();
  } catch (error) {
