...
watch(() => props.modelValue, (val) => {
  if (val) {
    formData.value = { kitchenName: "", description: "" };
    errors.value = { kitchenName: "" };
...
const validateField = (field) => {
  errors.value[field] = "";
  if (field === "kitchenName" && !formData.value.kitchenName) {
    errors.value.kitchenName = "Tên không được để trống.";
  }
};

const validateAll = () => {
  validateField("kitchenName");
  return !errors.value.kitchenName;
};
...
const save = async () => {
  ...
  try {
    const payload = {
      kitchenName: formData.value.kitchenName,
      description: formData.value.description
    };
    await kitchenStore.addKitchen(payload);
    toast.success("Thêm Bếp/Nơi pha chế thành công!");
    emit("saved", formData.value.kitchenName);
    close();
  } catch (error) {
