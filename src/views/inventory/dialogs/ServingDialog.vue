  ...
  methods: {
    ...
    validateField(field) {
      ...
      if (field === "name" && !this.formData.name) {
        this.errors.name = "Sở thích phục vụ không được để trống.";
      }
    },
    ...
    async submit() {
      try {
        const payload = {
          additionName: this.formData.name,
          extraPrice: Number(this.formData.price) || 0,
          description: this.formData.description,
        };
        
        const newPref = await this.servingStore.addPreference(payload);
        if (newPref) {
          this.$emit("saved", newPref);
