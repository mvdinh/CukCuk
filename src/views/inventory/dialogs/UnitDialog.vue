<template>
  <div class="ms-dialog-overlay" v-if="modelValue">
    <div class="ms-dialog unit-dialog">
      <div class="ms-dialog__header">
        <h3 class="ms-dialog__title">Thêm Đơn vị tính</h3>
...
export default defineComponent({
  ...
  methods: {
    ...
    validateField(field) {
      this.errors[field] = "";
      if (field === "unitName" && !this.formData.unitName) {
        this.errors.unitName = "Đơn vị tính không được để trống.";
      }
    },
    ...
    async submit() {
      try {
        await this.unitStore.addUnit(this.formData.unitName);
        toast.success("Thêm đơn vị tính thành công!");
        this.$emit("saved", this.formData.unitName);
        this.close();
      } catch (error) {
